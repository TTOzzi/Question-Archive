...
* 또 다른 이유는 아이폰의 그래픽 렌더링 방식입니다. 아이폰의 [그래픽스 파이프라인](https://ko.wikipedia.org/wiki/그래픽스_파이프라인)은 동기식으로 동작합니다.
  ...
  레이블의 텍스트를 업데이트하는 상황을 예로 들어보겠습니다. 먼저 화면에 표시할 레이블의 계층 구조를 인코딩하여 렌더 서버로 전송합니다. 전송된 레이블의 뷰 계층은 렌더 서버에서 디코딩되고, 렌더 서버는 GPU 에게 렌더링 요청을 합니다. 그런 다음 GPU 가 렌더링 작업을 시작합니다. [벡터 형태](https://ko.wikipedia.org/wiki/벡터_그래픽스)의 글꼴을 [래스터 형식](https://ko.wikipedia.org/wiki/래스터_그래픽스)으로 변환하고 텍스트를 픽셀로 변환합니다. 텍스트를 자르거나, 투명도를 적용할 때와 같이 해당 텍스트가 혼합된 레이어의 일부분일 경우 그래픽 렌더러는 보여줘야 할 레이블의 픽셀을 계산합니다. 렌더링 작업이 모두 완료되면 픽셀 정보들을 화면에 표시하게 됩니다.
